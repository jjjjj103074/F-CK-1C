...
local function AddElement(object)
    if object.parent_element == nil then object.parent_element = hmcs_root_name end
...
local function add_line(name, pos, length, width, rotation_deg, parent, element_params, controllers)
    ...
    if rotation_deg ~= nil then elem.init_rot = { rotation_deg, 0, 0 } end
...
local function add_param_text(name, value, pos, alignment, param_name, min_value, max_value, size)
    return add_text(name, value, pos, alignment, "font_hmcs_small", size, nil, { param_name }, { { "parameter_in_range", 0, min_value, max_value } })
end
...
local function add_heading_tape()
    ...
    for i = 1, heading_slot_count do
        ...
        add_line("hmcs_tape_tick_small_" .. tostring(i), { slot_x, 0.0 }, 0.008, line_width * 0.65, 90, tape.name, { tick_param }, { { "parameter_in_range", 0, 0.9, 1.1 } })
        add_line("hmcs_tape_tick_med_" .. tostring(i), { slot_x, 0.0 }, 0.012, line_width * 0.75, 90, tape.name, { tick_param }, { { "parameter_in_range", 0, 1.9, 2.1 } })
        add_line("hmcs_tape_tick_large_" .. tostring(i), { slot_x, 0.0 }, 0.016, line_width * 0.95, 90, tape.name, { tick_param }, { { "parameter_in_range", 0, 2.9, 3.1 } })
        ...
        for _, label_spec in ipairs(label_specs) do
            add_text("hmcs_tape_label_" .. tostring(i) .. "_" .. tostring(label_spec[1]), label_spec[2], { slot_x, 0.022 }, "CenterCenter", "font_hmcs_small", text_size_value, tape.name, { label_param }, { { "parameter_in_range", 0, label_spec[1] - 0.1, label_spec[1] + 0.1 } })
        end
    end
