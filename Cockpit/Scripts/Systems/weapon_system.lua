...
local function dlog(msg)
    if log and log.info then
        log.info("FCK1C WPN: " .. tostring(msg))
    end
end
...
local function dump_all_stations()
    ...
    for i = 0, NUM_STATIONS - 1 do
        ...
        if ok and info then
            ...
            dlog(string.format(
                "station[%d] CLSID=%s count=%s L2=%s L3=%s L4=%s",
                i,
                tostring(info.CLSID or "?"),
                tostring(info.count or "?"),
                tostring(w.level2 or "?"),
                tostring(w.level3 or "?"),
                tostring(w.level4 or "?")
            ))
        elseif not ok then
...
local function scan_aim9_stations()
    ...
    for i = 0, NUM_STATIONS - 1 do
        ...
        if ok and info and info.count and info.count > 0 then
            ...
            -- 方法 1: wsType 匹配
            if w and w.level2 == wsType_Missile and w.level3 == wsType_AA_Missile then
                is_aim9 = true
            end
            ...
            if not is_aim9 and info.CLSID then
                ...
                if clsid:find("AIM%-9") or clsid:find("AIM_9") or clsid:find("CATM%-9") then
                    is_aim9 = true
                end
            end
...
function update()
    ...
    if not is_aam or not aim9_selected then
        return
    end
    ...
    if debug_timer >= DEBUG_INTERVAL then
        ...
        dlog(string.format(
            "IR: stn=%d lock=%.1f az=%.4f el=%.4f fc=%d",
            selected_station, ir_lock, ir_az, ir_el, fc_mode
        ))
    end
end
