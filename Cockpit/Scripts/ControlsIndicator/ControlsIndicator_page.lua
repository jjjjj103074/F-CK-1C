...
local function add_line(name, pos, length, _width05, rot, parent, controllers)
    ...
    if _width05 ~= nil then width05 = _width05 end

    local elem = CreateElement("ceTexPoly")
    elem.name = name
    elem.material = "ARCADE"
    elem.vertices = { { 0, -width05 }, { 0, width05 }, { length, width05 }, { length, -width05 } }
    ...
    if rot ~= nil then elem.init_rot = { rot, 0, 0 } end

    if parent ~= nil then elem.parent_element = parent end

    if controllers ~= nil then elem.controllers = controllers end
...
local function add_text(name, value, pos, alignment, material, size, parent, element_params, controllers, formats)
    ...
    if parent ~= nil then elem.parent_element = parent end
    if element_params ~= nil then elem.element_params = element_params end
    if controllers ~= nil then elem.controllers = controllers end
    if formats ~= nil then elem.formats = formats end
...
base.vertices = {
    { -(size_x + rud_shift + rud_base_shift + 2 * ds), -(size_y_negative + 2 * ds + rudder_shift) }, -- lower left, CW
...
base.controllers = { { "show" }, { "screenspace_position", 2, -(aspect - 2 * size_x), 0 }, { "screenspace_position", 1, 0, 0 } }
...
stick_position.vertices = { { -stick_index_size, -stick_index_size }, { -stick_index_size, stick_index_size }, { stick_index_size, stick_index_size }, { stick_index_size, -stick_index_size } }
...
stick_position.controllers = { { "stick_pitch", -size_y_negative }, { "stick_roll", size_x } }
...
trimmer10.vertices = { { -rud_shift * 0.5, -line_width }, { -rud_shift * 0.5, line_width }, { rud_shift * 0.5, line_width }, { rud_shift * 0.5, -line_width } }
...
trimmer10.controllers = { { "move", -10.0 / 17.0 * size_y_negative }, { "rotate", math.rad(90) }, { "scale", 0.7, 1.0 } }
...
-- THROTTLE
local throttle_index = add_line("throttle_index", { throttle_px - rud_shift * 1.5 * 0.5, throttle_py }, rud_shift * 1.5, line_width * 2, nil, base.name, {
    { "throttle", throttle_scale_length } --[[,{"scale",1.0,2.0}]],
})
...
for i = 1, 2 do
    ...
    wheel_brake_mask.vertices = { { 0, 0 }, { 0, sz_wheel_brake }, { -signum[i] * 0.3 * sz_wheel_brake, sz_wheel_brake }, { -signum[i] * 0.3 * sz_wheel_brake, 0 } }
    ...
    wheel_brake.vertices = { { 0, 0 }, { 0, sz_wheel_brake }, { -signum[i] * 0.3 * sz_wheel_brake, sz_wheel_brake } }
    ...
    wheel_brake_actual.vertices = { { 0, -0.05 * sz_wheel_brake }, { 0, 0.05 * sz_wheel_brake }, { -signum[i] * 0.3 * sz_wheel_brake, 0.05 * sz_wheel_brake }, { -signum[i] * 0.3 * sz_wheel_brake, -0.05 * sz_wheel_brake } }
