...
extern "C"
{
	...
	*/
	F_CK_1C_EFM_API void ed_fm_set_surface(double		h,//surface height under the center of aircraft
	...
	*/
	F_CK_1C_EFM_API void ed_fm_refueling_add_fuel(double fuel);
	...
	*/
	...
	F_CK_1C_EFM_API void ed_fm_set_fc3_cockpit_draw_args(EdDrawArgument* drawargs, size_t size);

	/*
	shake level amplitude for head simulation ,
	*/
	F_CK_1C_EFM_API double ed_fm_get_shake_amplitude();
	...
	*/
	F_CK_1C_EFM_API void ed_fm_release();

	/*
	various param call back to proper integrate your FM to DCS , like engine RPM , thrust etc
	*/
	F_CK_1C_EFM_API double ed_fm_get_param(unsigned param_enum);
	...
	*/
	F_CK_1C_EFM_API bool ed_fm_make_balance(double& ax,//linear acceleration component in world coordinate system);
		double& ay,//linear acceleration component in world coordinate system
		double& az,//linear acceleration component in world coordinate system
		double& vx,//linear velocity component in world coordinate system
		double& vy,//linear velocity component in world coordinate system
...
*/
	...
	*/
	F_CK_1C_EFM_API void ed_fm_set_plugin_data_install_path(const char* path);

	// damages and failures
	// callback when preplaneed failure triggered from mission 
	F_CK_1C_EFM_API void ed_fm_on_planned_failure(const char* data);

	// callback when damage occurs for airframe element 
	F_CK_1C_EFM_API void ed_fm_on_damage(int Element, double element_integrity_factor);

	// called in case of repair routine 
	F_CK_1C_EFM_API void ed_fm_repair();

	// in case of some internal damages or system failures this function return true , to switch on repair process
	F_CK_1C_EFM_API bool ed_fm_need_to_be_repaired();

	// inform about  invulnerability settings
	F_CK_1C_EFM_API void ed_fm_set_immortal(bool value);
	// inform about  unlimited fuel
	F_CK_1C_EFM_API void ed_fm_unlimited_fuel(bool value);
	// inform about simplified flight model request 
	F_CK_1C_EFM_API void ed_fm_set_easy_flight(bool value);

	// custom properties sheet 
	F_CK_1C_EFM_API void ed_fm_set_property_numeric(const char* property_name, float value);
	// custom properties sheet 
	F_CK_1C_EFM_API void ed_fm_set_property_string(const char* property_name, const char* value);
	...
	*/
	...
	F_CK_1C_EFM_API bool ed_fm_push_simulation_event(ed_fm_simulation_event& in);
