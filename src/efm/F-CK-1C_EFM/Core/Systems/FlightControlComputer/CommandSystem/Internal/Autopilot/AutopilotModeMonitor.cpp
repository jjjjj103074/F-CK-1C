...
// Private AFCS monitor implementation; use FlightControlCommandSystem.
	...
	: config_(config)
{
	const bool valid = config_.pitch_tracking_error_limit_rad > 0.0 &&
		config_.vertical_speed_tracking_error_limit_ft_s > 0.0 &&
	...
	const AutopilotModeMonitorObservation& observation)
{
	return std::isfinite(observation.dt_s) && observation.dt_s > 0.0 &&
		std::isfinite(observation.pitch_tracking_error_rad) &&
		std::isfinite(observation.vertical_speed_tracking_error_ft_s) &&
	...
	const AutopilotModeMonitorObservation& observation) const
{
	...
	if (observation.vertical_type ==
		VerticalGuidanceReferenceType::PitchAttitude)
	{
		return std::fabs(observation.pitch_tracking_error_rad) >
			config_.pitch_tracking_error_limit_rad;
	}
	return std::fabs(observation.vertical_speed_tracking_error_ft_s) >
		config_.vertical_speed_tracking_error_limit_ft_s;
}
